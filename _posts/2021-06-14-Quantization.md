---
layout: post
title:  "Pytorch Quantization Of YOLOv3 Family"
description: "Pytorch Quantization applied to YOLOv3 and YOLOv3 tiny models."
type: card-dated
date: 2021-06-14 20:19:00 +0900
image: http://placehold.it/750X300?text=Header+Image # for local images, place in /assets/img/posts/
caption: 
last-updated: 2021-06-14 20:19:00 +0900
categories: post
tag: Pytorch
author: Jose
card: card-2
...
In this document I will go through the process of quantize an existing YOLOv3 model using Pytorch.
Please note that nowadays, May 2021, Pytorch does not support quantization using GPU!
...
As stated in the introduction, nowadays, Pytorch only support quantization using CPU. Because of that, you have to
...
So I replaced the `leakyRELU` by plain `RELU`.
After this replacement, I retrained the model for a few epochs just to adapt to the new component.
This is an important step! Afterwards, check the model's accuracy, it should not have changed that much.
...
Other than replacing the operator with a similar one like we did with `leakyRELU` and `RELU`,
...
The name of functions for that purpose are: `DeQuantStub` and `QuantStub`.
...
Now, that the forward flow is correct, let's take a step further.
...
Pytorch will take those operations and merge them in a single one such as `ConvReLU2d` (Conv2d + ReLU).
...
It has to be at class level because this operation will be calibrated so it has to save statistics. It can't be created 'on the fly'.

When executing the addition operation, you can use it like this:

`x = self.f_add.add(x, previous_x)`

Of course you have to pay attention that the datatypes of the tensors `x` and `previous_x` match!
...
`fbgemm` (for use on x86, https://github.com/pytorch/FBGEMM) and `qnnpack` (for use on the ARM QNNPACK library https://github.com/pytorch/QNNPACK).
...
7. Convert the model to quantized model
8. Evaluate the model to check accuracy regarding baseline
9. Save the quantized model

In the step 6, depending on your quantization method, you will calibrate or do QAT training.
...
Apart from using `qnnpack` configuration, you can also take a few measures.
I particularly like this link: https://pytorch.org/tutorials/recipes/mobile_perf.html
